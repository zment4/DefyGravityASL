...
//	settings.Add("twl", false, "GameTime is loadless instead of IGT");
	...
	vars.scannerTask = new System.Threading.Thread(() => {});
}

init {
	if (vars.scannerTask.IsAlive) {
		vars.cancelRequested = true;
		while (vars.scannerTask.IsAlive) Thread.Sleep(10);
	}
...
update {
	if (settings["debug"]) print((!vars.scannerTask.IsAlive ? "[DGASL] Scanning complete" : "[DGASL] Scanning for address"));
	
	if (vars.scannerTask.IsAlive) return false;
